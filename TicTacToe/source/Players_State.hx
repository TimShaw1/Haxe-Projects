...
class Players_State extends FlxState
{
	...
	override public function update(elapsed:Float):Void
	{
		...
		else
		{
			if (!game_over)
			{
				var win_string = check_win(board) == 0 ? "Tie" : check_win(board) == 1 ? "X wins" : "O wins";
	...
	public function place_sprite_on_tile(tile:FlxSprite, index:Int):Void
	{
		...
		if (Xturn)
		{
			...
			// Set board on this tile to be X
			board[index] = 1;
		}
		else
		{
			...
			// Set board on this tile to be O
			board[index] = -1;
		}
	...
	/** returns true if someone won or there are no spaces left */
